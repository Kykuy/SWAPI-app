...
const Output = (props) => { 
  ...
  let finalOutput = props.selected !== 'films' ?
   filtered.filter( item => item.name.toLowerCase().includes( props.searchInput.toLowerCase() ) ) :
   filtered.filter( item => item.title.toLowerCase().includes( props.searchInput.toLowerCase() ) );
